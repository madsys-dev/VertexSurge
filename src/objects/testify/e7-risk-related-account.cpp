...
int main(int argc, char *argv[]) {
  ...
  auto ve12 = shared_ptr<BigOperator>(new VExpand(
      "Account", "Account", "transfer", Direction::Forward, k_min, k_max,
      ExpandPathType::Shortest, ExpandStrategy::Dense, "a.id", "b.id"));
  ve12->addLastDependency(scan_account1);

  // auto cc12 =
  //     shared_ptr<BigOperator>(new MatrixColumnCount("count",
  //     SortOrder::DESC));
  ...
  // root->results->dump_to_json("/tmp/results.json");
