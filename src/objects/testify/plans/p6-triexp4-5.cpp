...
int main(int argc, char *argv[]) {
  ...
  auto p13C =
      shared_ptr<BigOperator>(new TableConvert(TableType::ColMainMatrix, true));
  p13C->addLastDependency((p13));

  auto m132 = shared_ptr<BigOperator>(new Mintersect("c.id", "Person", false));

  m132->addDependency(p12);
  m132->addDependency(p23C);
  m132->addLastDependency(p13C);

  auto p41C =
      shared_ptr<BigOperator>(new TableConvert(TableType::ColMainMatrix, true));
  p41C->addLastDependency((p41));

  auto bj1234 = shared_ptr<BigOperator>(new HashJoin("a.id", only_count));
  bj1234->addDependency(m132);
  bj1234->addLastDependency(p41C);
