...
/*
  Double Triangle with a shared edge 1-3
  1-2 1-3 1-4 2-3 3-4
  This plan uses binary join then intersect
*/
...
int main(int argc, char *argv[]) {
  ...
  auto p41 =
      vexpand_knows(scan_person4, scan_person1, "d.id", "a.id", k_min, k_max);
  auto p41C = shared_ptr<BigOperator>(
      new TableConvert(TableType::ColMainMatrix, false));
  p41C->addLastDependency(p41);

  auto bj412 = shared_ptr<BigOperator>(new HashJoin("a.id", false));
  bj412->addDependency(p21C);
  bj412->addLastDependency(p41C);
  ...
  m1234->addDependency(bj412);
  m1234->addDependency(p31C);
  m1234->addDependency(p32C);
  m1234->addLastDependency(p34C);
