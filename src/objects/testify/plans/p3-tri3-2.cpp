...
/*
    ...
    This plan uses hash join to get 1-2-3 then semi join with 1-3
*/
...
int main(int argc, char *argv[]) {
  ...
  auto ve12_T = shared_ptr<BigOperator>(
      new TableConvert(TableType::ColMainMatrix, false));
  ve12_T->addLastDependency(p12);

  auto ve32_T =
      shared_ptr<BigOperator>(new TableConvert(TableType::ColMainMatrix, true));
  ve32_T->addLastDependency((p23));

  //   auto ve13_T =
  //       shared_ptr<BigOperator>(new TableConvert(TableType::ColMainMatrix,
  //       true));
  //   ve13_T->addLastDependency((p13));

  auto p123 = shared_ptr<BigOperator>(new HashJoin("b.id", false));
  p123->addDependency(ve12_T);
  p123->addLastDependency(ve32_T);

  auto t123 =
      shared_ptr<BigOperator>(new SemiJoin<2>({"a.id", "c.id"}, only_count));
  t123->addDependency(p123);
  t123->addLastDependency(p13);
